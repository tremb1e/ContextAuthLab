...
| ②  端侧脱敏（可云端更新） | 在不破坏认证特征的前提下，按规则过滤 PII；规则可热更新 |
...
由 idea step④，路由 key 应是「app 包名 + 脱敏后 UI 前端信息」。可拆为：

| 路由维度 | 当前是否有 | 备注 |
|---|---|---|
| `app_package_name`（明文） | ✅ | `Batch.app_package_name`、每 context event 也带 |
| `foreground_activity_class_name`（明文） | ✅ | 来源不稳定（仅 `WINDOW_STATE_CHANGED`） |
...
| `viewIdResourceName` 明文 | ✅ | OK | OK |
| node `text` 明文（非 PII） | ✅（启发式保留） | 受 max_text_length=128 限制 | OK |
| `contentDescription` 明文 | ❌（强 `<TEXT_REDACTED>`） | 牺牲了一部分语义 | 不太影响认证 |
...
| 规则热更新 | ✅ `/api/v1/rules` + canonical SHA-256 | – | OK |
...
**结论：当前脱敏对认证模型几乎是温和的，但对 TYPING 专家近乎致命，对 TAP/SCROLL 完全致命。**

| 脱敏行为 | 对认证的影响 |
|---|---|
| Password 节点整棵丢弃 | 良性（敏感性合规要求） |
| Editable text 替换为 `<EDITABLE_TEXT_DROPPED>` | 良性（仍保留 editable 状态、IME 可见性） |
| `text` 字段保留（仅模式替换） | 良性，UI 语义保留 |
| `content_desc_redacted = <TEXT_REDACTED>` 全替换 | 损失了部分语义信号；但 `viewIdResourceName` 还在，影响小 |
...
| `max_text_length=128` | 长文本被丢弃，影响阅读场景的"文本量"特征——但可通过节点数与字符总数估算 |

注意：脱敏策略的"温和"与"致命"是相对认证特征而言。从隐私合规视角，当前策略完全符合 `requirements_matrix.md` APP-09 的硬约束，**这正是必须直面的张力**。
...
按"必须新增 / 建议增强 / 可选优化"三档列出。每条注明：理由、实现位置、隐私影响与对应脱敏策略、优先级与预估难度。
...
| E5 | **`contentDescription` 的弱保留**：除模式替换外，记录"该节点 contentDescription 长度、是否存在、字符类型分布"等元统计 | 视频类 / 表单类语义补强 | `RedactionEngine.sanitizeNode` 增加元统计字段 | 不增加新明文 | P2 | 低 |
...
| O6 | 在 `redaction_summary` 中加入 dynamic rules 命中率分布 | 调参用 | 无 | P3 |
