...
`{batch_id}.json` stores decompressed, redacted batch JSON. `{batch_id}.meta.json` stores envelope metadata without `payload_base64`, ingest time, sizes, and schema validation result.
