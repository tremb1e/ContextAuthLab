...
- Android starts collection automatically when consent, Accessibility, battery optimization, notification permission, server health, ClockSync, screen state, and the Wi-Fi policy are all ready.
...
Not implemented in this stage: authentication models, context routing models, MoE, training code, inference code, non-empty production cloud redaction rules, or server frontend pages.
...
- Raw input field text is dropped. Password nodes are dropped. Fixed-format sensitive strings are redacted before upload.
- `/api/v1/rules` currently returns an empty dynamic rule list and empty package blocklist; the client still applies built-in baseline redaction and can apply future server-provided rules.
- Server ingest rejects batches that carry raw UI keys such as `text`, `contentDescription`, `package_name`, `view_id`, or `window_title`, and requires `diagnostics.redaction_applied: true`.
